...
class TestOrderMatchkeys(object):
    def test_expected(self, spark):
        ...
        testDf = pd.DataFrame(
            {
                "mks": mks,
                "count": [(dfo.join(dffn, on=mk, how="inner")).count() for mk in mks],
            }
        )
...
class TestDemographics:
    def test_expected(self, spark):
        ...
        result_df_raw = demographics(
            *["sex", "age_group"], df=test_df_raw, identifier="id"
        )

        assert_df_equality(
            intended_data_raw, result_df_raw, ignore_nullable=True, ignore_schema=True
        )
        ...
        result_df_linked = demographics(
            *["sex", "age_group"], df=test_df_linked, identifier="id"
        )

        assert_df_equality(
            intended_data_linked,
            result_df_linked,
...
class TestDemographicsCompare:
    def test_expected(self, spark):
        ...
        test_raw = demographics(*["sex", "age_group"], df=df_raw, identifier="id")

        test_linked = demographics(*["sex", "age_group"], df=df_linked, identifier="id")
        ...
        assert_df_equality(
            intended_data, result_df, ignore_nullable=True, ignore_schema=True
        )
        ...
        test_dem_raw1 = demographics(*["sex", "age_group"], df=df_raw, identifier="id")

        test_dem_linked1 = demographics(
            *["sex", "age_group"], df=df_linked, identifier="id"
        )
        ...
        assert_df_equality(
            intended_data1, result_df1, ignore_nullable=True, ignore_schema=True
        )
...
class TestMatchkeyDataframe(object):
    def test_expected(self, spark):
        ...
        intended_data = [
            [
                1,
                "[(first_name=first_name),(last_name=last_name),(uprn=uprn),(date_of_birth=date_of_birth)]",
            ],
            [
                2,
                "[(substring(first_name,0,2)=substring(first_name,0,2)),(substring(last_name,0,2)=substring(last_name,0,2)),(uprn=uprn),(date_of_birth=date_of_birth)]",
            ],
        ]
